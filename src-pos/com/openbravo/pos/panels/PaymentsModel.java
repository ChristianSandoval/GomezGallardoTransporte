    ...
    public static PaymentsModel loadInstance(AppView app, String cash) throws BasicException {
        ...
        Object[] valtickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT COUNT(*), SUM(PAYMENTS.TOTAL) " +
              ...
              "WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEY = ? AND PAYMENTS.PAYMENT<>'debt' AND PAYMENTS.PAYMENT<>'magcard' AND PAYMENTS.PAYMENT<>'debtpaid'"
...
String dateNow = formatter.format(currentDate.getTime());
    ...
    public static PaymentsModel loadInstance(AppView app) throws BasicException {
        ...
        Object[] valtickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT COUNT(*), SUM(PAYMENTS.TOTAL) " +
              "FROM PAYMENTS, RECEIPTS " +
              "WHERE PAYMENTS.PAYMENT<>'free' AND PAYMENTS.PAYMENT<>'magcard' AND PAYMENTS.PAYMENT<>'debt' AND PAYMENTS.PAYMENT<>'debtpaid' AND PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEY = ?"
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.INT, Datas.DOUBLE}))
        ...
        List products = new StaticSentence(app.getSession(), 
              "SELECT TICKETS.TICKETID,RECEIPTS.DATENEW,PRODUCTS.NAME,CUSTOMERS.SEARCHKEY,CUSTOMERS.NAME,TICKETLINES.UNITS*TICKETLINES.PRICE,PEOPLE.NAME,CUSTOMERS.NOTES FROM TICKETLINES INNER JOIN TICKETS ON TICKETLINES.TICKET=TICKETS.ID INNER JOIN RECEIPTS ON RECEIPTS.ID=TICKETS.ID INNER JOIN PRODUCTS ON PRODUCTS.ID=TICKETLINES.PRODUCT INNER JOIN PEOPLE ON PEOPLE.ID=TICKETS.PERSON INNER JOIN CUSTOMERS ON CUSTOMERS.ID=TICKETS.CUSTOMER WHERE RECEIPTS.MONEY = ?", 
...
public static class ProductSalesLine
    implements SerializableRead
  {
    ...
    public void readValues(DataRead dr)
      throws BasicException
    {
      this.tick=dr.getInt(1);
      this.fecha=dr.getTimestamp(2);
      this.m_ProductName = dr.getString(3);
      this.idcte = dr.getString(4);
      this.cliente = dr.getString(5);
      this.m_Total = dr.getDouble(6);
      this.usuario = dr.getString(7);
      this.mensualidad = dr.getString(8);
    }
    
    public String printTicket()
    {
      return Formats.INT.formatValue(this.tick);
    }
    public String printFecha() {
        return Formats.TIMESTAMP.formatValue(fecha);
    }
    public String printUsuario() {
        return usuario;
    }
    public String printMensualidad() {
        return mensualidad;
    }
    public String printIdCte() {
        return idcte;
    }
    public String printCte() {
        return cliente;
    }
    public String printProductName()
    {
