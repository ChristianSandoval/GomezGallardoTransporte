...
public CustomerInfoExt findCustomerExtById(String id)
    ...
    public SentenceList getTicketsList(String location) {
         return new StaticSentence(s
            , new QBFBuilder(
            ...
            "WHERE "+ (location.equals("0")? "": "T.STATUS="+location+" AND ") +"?(QBF_FILTER) GROUP BY T.ID, T.TICKETID, T.TICKETTYPE, R.DATENEW, P.NAME, C.NAME ORDER BY R.DATENEW DESC, T.TICKETID", new String[] {"T.TICKETID", "T.TICKETTYPE", "PM.TOTAL", "R.DATENEW", "R.DATENEW", "P.NAME", "C.NAME"})
    ...
    public final TicketInfo loadTicket(final int tickettype, final int ticketid) throws BasicException {
        ...
        if (ticket != null) {
            ...
            ticket.setLines(new PreparedSentence(s
                , "SELECT L.TICKET, L.LINE, L.PRODUCT, L.ATTRIBUTESETINSTANCE_ID, L.UNITS, L.PRICE, T.ID, T.NAME, T.CATEGORY, T.VALIDFROM, T.CUSTCATEGORY, T.PARENTID, T.RATE, T.RATECASCADE, T.RATEORDER, L.ATTRIBUTES, L.MENSUALIDAD " +
            ...
            public Object transact() throws BasicException {
                ...
                new PreparedSentence(s
                    ...
                    , SerializerWriteParams.INSTANCE
                        ...
                        setInt(6, Integer.parseInt(ticket.getProperty("sucursal")));
                    }});

                SentenceExec ticketlineinsert = new PreparedSentence(s
                    , "INSERT INTO TICKETLINES (TICKET, LINE, PRODUCT, ATTRIBUTESETINSTANCE_ID, UNITS, PRICE, TAXID, ATTRIBUTES,MENSUALIDAD) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)"
